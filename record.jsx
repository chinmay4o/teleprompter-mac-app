...
function RecordingScreen({ script, session, setSession, tweaks, onBack, onPrompter }) {
  ...
  useEffectRec(() => {
    let stream;
    let cancelled = false;
    // If getUserMedia hangs (macOS permission dialog not shown / denied silently),
    // fall back to 'off' after 2s so the UI never stays stuck at "Connecting…"
    const timeout = setTimeout(() => { if (!cancelled) setCam('off'); }, 2000);
    (async () => {
      try {
      ...
      } catch (e) { clearTimeout(timeout); if (!cancelled) setCam('off'); }
    })();
  ...
  return (
    <div style={{ position: 'relative', height: '100%', background: '#0b0d12', overflow: 'hidden', minHeight: 0 }}>
      ...
      <div style={{ position: 'absolute', top: 56, left: 16, zIndex: 25, display: 'flex', gap: 8, alignItems: 'center' }}>
        <DevicePill icon="camera" label={cam === 'live' ? 'FaceTime HD Camera' : cam === 'off' ? 'No camera' : 'Connecting…'} />
        <DevicePill icon="mic" label="MacBook Pro Mic" />
...
function DevicePill({ icon, label }) {
  return (
    <span style={{ display: 'inline-flex', alignItems: 'center', gap: 6, fontFamily: 'var(--font-mono)',
      fontSize: 11, color: 'rgba(255,255,255,0.85)', background: 'rgba(0,0,0,0.4)', padding: '4px 9px',
      borderRadius: 999, border: '1px solid rgba(255,255,255,0.12)' }}>
      <Icon name={icon} size={13} />{label}<Icon name="chevron" size={11} style={{ opacity: 0.6 }} />
    </span>
  );
}
